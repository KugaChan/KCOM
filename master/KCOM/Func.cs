...
namespace KCOM
{
    ...
    public class eFIFO_string
    {
        ...
        public void Init(int _max_buffer_len, int _max_queue_depth)
        {
            buffer_data = new List<string>();
            buffer_op = new List<int>();
        ...
        public string Output(ref int value)
        {
            string data;
            ...
            {
                data = buffer_data[bottom];
                value = buffer_op[bottom];

#if SUPPORT_SHOW_FIFO_DATA
                Console.WriteLine("out:{0}({1}:{2})", value, top, bottom);
                for(int i = 0; i < buffer_op[bottom]; i++)
                {
                    Console.Write(" {0}", buffer_data[bottom][i]);
                }
                Console.WriteLine("({0}:{1})", top, bottom);
        ...
        public void Input(string data, int value)
        {
            object locker = new object();
