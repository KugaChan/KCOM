    ...
    class SCOM
    {
        ...
        void RcvData_Handle(byte[] buffer, int length)
        {
            ...
            if( (SCOM.run_ecmd == false) || 
                (ext_name == null) || 
                ( (Func.Char_String_compare(ext_name.ToCharArray(), "cmd", 3) == false) ) )
            {
                string append_str = "";
            ...
            {
                if(Func.Char_String_compare(ext_name.ToCharArray(), "cmd", 3) == true)
                {
